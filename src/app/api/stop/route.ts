...
const MANAGEMENT_API_URL = 'http://168.181.187.83:3000/api/stop';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!secret) {
      console.error('[API Stop] JWT_SECRET no está configurado.');
      return NextResponse.json(
    ...
    console.log(`[API Stop] Response from management API:`, responseText);
    try {
    ...
    console.error('[API Stop] Error:', error.message);
