...
const MANAGEMENT_API_URL = 'http://168.181.187.83:9000/api/delete_db';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!secret) {
      console.error('[API DeleteDB] JWT_SECRET no está configurado.');
      return NextResponse.json(
    ...
    console.error('[API DeleteDB] Error:', error.message);
